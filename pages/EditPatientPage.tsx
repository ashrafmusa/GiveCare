...
const EditPatientPage: React.FC = () => {
  ...
  const { getPatientById, updatePatient } = usePatientContext();
  ...
  if (!patient) {
    return <Card><p className="text-center text-gray-500">Patient not found.</p></Card>;
  }

  return (
    <Card>
      <h1 className="text-2xl font-bold mb-4">Edit Patient: {patient.name}</h1>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-end space-x-2">
            <Button type="button" variant="secondary" onClick={() => navigate(`/patient/${id}`)} disabled={isSubmitting}>
                Cancel
            </Button>
